...
max_tweets = 1000
searched_tweets = f_mine.mine(api, '#BernieOrBust', max_tweets)
...
with con:
	...
	cur.execute("CREATE TABLE Sanders( \
		...
		tweet TEXT, \
		UNIQUE(screen_name))")
	cur.executemany("INSERT OR IGNORE INTO Sanders(screen_name, tweet) VALUES(?, ?)", format_sql) # Execute many statements
