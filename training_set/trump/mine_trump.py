...
max_tweets = 1000
searched_tweets = f_mine.mine(api, '#TrumpTrain', max_tweets)
...
with con:
	...
	cur.execute("CREATE TABLE Trump( \
		...
		tweet TEXT, \
		UNIQUE(screen_name))")
	cur.executemany("INSERT OR IGNORE INTO Trump(screen_name, tweet) VALUES(?, ?)", format_sql) # Execute many statements
